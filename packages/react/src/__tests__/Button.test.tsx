...
describe("<Button>", () => {
  it("Should render `button` with correct class name from type prop", () => {
    ...
    expect(buttonElement[0].getAttribute('class')).toEqual(expect.stringContaining(buttonArgs.primary.type));
  });
  ...
  it("Should render `button` with correct size class name from prop size", () => {
    ...
    const buttonElement = screen.getAllByRole("button");
    expect(buttonElement).not.toBeNull();
    expect(buttonElement[0].getAttribute('class')).toEqual(expect.stringContaining(buttonArgs.primary.size));
  });
